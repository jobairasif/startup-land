import React from "react";

export default function Support() {
  return (
    <section className="px-4 p-40">
      <div className="container mx-auto flex justify-center gap-36">
        <div className="flex gap-8">
          <img
            className="w-[80px] h-fit"
            src="/images/support-icon.svg"
            alt=""
          />

          <div>
            <h3 className="text-xl font-bold mb-4 flex gap-20 items-center">
              Do you need help support{" "}
              <svg
                width="22"
                height="16"
                viewBox="0 0 22 16"
                fill="none"
                xmlns="http://www.w3.org/2000/svg"
              >
                <path
                  opacity="0.3"
                  fill-rule="evenodd"
                  clip-rule="evenodd"
                  d="M21.6499 7.23588L14.5149 0.316535C14.0796 -0.105512 13.3735 -0.105512 12.9383 0.316535C12.5028 0.739114 12.5028 1.42351 12.9383 1.84556L18.1701 6.91921H1.11477C0.499168 6.91921 0 7.40308 0 8.0004C0 8.59717 0.499168 9.08158 1.11477 9.08158H18.1701L12.9384 14.1552C12.5029 14.5767 12.5029 15.2617 12.9384 15.6837C13.156 15.8945 13.4415 15.9999 13.7268 15.9999C14.0121 15.9999 14.2974 15.8945 14.515 15.6837L21.6499 8.76437C22.0853 8.34288 22.0853 7.65793 21.6499 7.23588Z"
                  fill="#0F2137"
                />
              </svg>
            </h3>
            <p>
              Get your website ads tests delivered at let <br /> collect sample
              from the victory of the update <br /> managments services.
            </p>
          </div>
        </div>
        <div className="flex gap-8">
          <img
            className="w-[80px] h-fit"
            src="/images/support-icon.svg"
            alt=""
          />

          <div>
            <h3 className="text-xl font-bold mb-4 flex gap-20 items-center">
              Do you need help support{" "}
              <svg
                width="22"
                height="16"
                viewBox="0 0 22 16"
                fill="none"
                xmlns="http://www.w3.org/2000/svg"
              >
                <path
                  opacity="0.3"
                  fill-rule="evenodd"
                  clip-rule="evenodd"
                  d="M21.6499 7.23588L14.5149 0.316535C14.0796 -0.105512 13.3735 -0.105512 12.9383 0.316535C12.5028 0.739114 12.5028 1.42351 12.9383 1.84556L18.1701 6.91921H1.11477C0.499168 6.91921 0 7.40308 0 8.0004C0 8.59717 0.499168 9.08158 1.11477 9.08158H18.1701L12.9384 14.1552C12.5029 14.5767 12.5029 15.2617 12.9384 15.6837C13.156 15.8945 13.4415 15.9999 13.7268 15.9999C14.0121 15.9999 14.2974 15.8945 14.515 15.6837L21.6499 8.76437C22.0853 8.34288 22.0853 7.65793 21.6499 7.23588Z"
                  fill="#0F2137"
                />
              </svg>
            </h3>
            <p>
              Get your website ads tests delivered at let <br /> collect sample
              from the victory of the update <br /> managments services.
            </p>
          </div>
        </div>
      </div>
    </section>
  );
}
